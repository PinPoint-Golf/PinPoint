...
| OpenSSL (PPCP transport, `HAVE_PPCP_TRANSPORT`) | ≥ 1.1.1 (system) | Apache-2.0 | Apache-2.0 is compatible with GPL-3.0. Needed for a TLS **external** pre-shared key: `PPCP-RV` §8 records that the PSK interfaces of common toolkits, Qt's included, do not reach a TLS 1.3 external PSK, so the transport drives OpenSSL's external-PSK session callbacks directly. Dynamically linked against the system or Homebrew build; not redistributed by any of the three packaging paths. Optional at build time — an absent OpenSSL yields an app with no PPCP transport, never an unencrypted one. |
