...
| **Launch monitor** | `src/LaunchMonitor/tests` | 2 | `LastShot.CSV` parsing (columns matched by name not position, metric **and** imperial units as FSX2020 declares them, the three derived values, malformed/torn/short input) and shot attribution (`ShotPairing` arming and displacement, parking a reading until a `swingDir` exists, and the watermark that stops a stale file being claimed at startup). GoogleTest; the pairing half writes its own fixtures into a `QTemporaryDir` and needs Qt6 Test for `QSignalSpy`. |
