...
| B4 | norms.json header — no cohort content, and why | ☑ complete — 80/80 | 2026-07-28 |
| — | **package closed** — Part A + Part B in, all four gates met | ☑ | 2026-07-28 |
...
| 2026-07-28 | B4 | **Package close, docs and content only.** norms.json's header now carries the cohort schema, the CLOSED age vocabulary and why it can never be extended by pack content, the context-major probe order with both of its intended consequences, and the two doctrine rules the brief asked for: **cohort shifts the corridor and NEVER inverts valence** (same rule as context — a row that flipped the direction would be a different measure wearing the same id), and **band-edge cliffs are accepted** with no interpolation, because interpolating implies a continuity the banded literature does not support. The key line is reworded to `(measure, context, cohort) — NEVER BY ATHLETE ID`, matching `norm.h`. Where content will come from is recorded **so nobody pads cohorts for symmetry**: the ROM family, because that is what the literature supports and because 16 shipped rows already rest on the male-only professional curves in 10.1123/jab.27.3.242 with nothing in the schema saying so — while the setup measures are largely cohort-INVARIANT (already body-normalised) and smash is self-normalising, so a cohort row authored on those to look even-handed is one nobody can defend. `norm.h` gained the same two doctrine rules beside the `Cohort` type. **Ledger N25 closed in the same change**: the developer guide's `Norm` snippet predated shape, plausibility AND cohort, §4 described a resolution walk with no cohort probe, and §12's suite table was missing `seed_conversion_test` — all three swept. 80/80. Next: nothing — the package is closed. Open items are N29 (grading-time cohort, deferred to *Diagnosis execution, V&V* with N5) and the author's-call shape candidates (N1–N3, N20). |
