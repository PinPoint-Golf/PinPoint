...
| `[~]` | PPS | ✅ **The race is resolved by TAKEOVER instead (see Log) — suppression is no longer the mechanism.** Residual: the `onDeclare()` backstop on `counterpartId`, and deciding whether per-pairing suppression is still wanted at all. ~~§6.1 arbitration: per-pairing mDNS advertisement suppression~~, **suppressed before dialling**, restored on **any** ending. Plus the `onDeclare()` backstop keyed on `counterpartId` (not `pairingId`) via the existing `peerForId()` `:453`. |
