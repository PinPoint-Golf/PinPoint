...
| Clause | |
|---|---|
| A persisted pairing reconnects over the cable, no QR | ✅ **met** 29 Aug 2026 — but the link FLAPS, see Log |
...
| `min_rtt` / `offset_sigma` recorded for Linux | ✅ **met** — 0.670 ms / 2.754 ms, see Log; ⚠ sigma is WORSE than WiFi's 1.16 ms |
...
| 2026-08-29 | 2L | ⚠ **THE CABLE BEATS WiFi ON RTT AND LOSES ON SIGMA, WHICH IS NOT WHAT THE DESIGN ARGUMENT PREDICTS.** Phase 0's WiFi arm measured `min_rtt` 1.64 ms → `offset_sigma` 1.16 ms. The cable more than halves RTT (0.670 ms, floor 0.335 ms) and yet lands at `offset_sigma` **2.754 ms** — 2.4× WORSE, and eight times its own floor. §1's arithmetic (`offset_sigma` floored at half min RTT) says the transport is the only lever; on this host it plainly is not the binding one, so something other than RTT dominates the estimator here. ⛔ Do not quote the cable as a sigma improvement on Linux until this is understood — the accuracy case for wired is **not** reproduced on this platform. |
