...
**Everything is uncommitted**, per the standing rule — nothing has been committed or pushed.
...
| `X6` | `norm_model_test` ("the pack-level function returns the same measure the façade does") failed exactly **twice**, both times in a shell command of the form `cmake --build … ; ctest …` where the build had just relinked that binary. Six subsequent full runs, plus isolated runs and a run of the suspected-ordering pair, are all green. The evidence points at ctest launching a binary the linker had not finished writing — a race in the INVOCATION, not a defect in the code — and the mitigation is to run the build to completion before starting ctest. Left open rather than closed: the diagnosis fits both occurrences but has not been proven. | open |
