...
#include "hm_session_types.h"       // ReferenceAnchor / HistoryResult — measured by the
                                    // session worker, aliased below as HmInstance::…
...
public:
    ...
    //
    // ⚠ THE TYPE LIVES WITH THE THING THAT MEASURES IT (src/IMU/hm_session_types.h).
    // HmSessionWorker drains the presence event and fills this in; this class only
    // carries it across the thread boundary, so nesting it here made device-layer
    // code depend on the GUI controller that displays its output. The alias exists
    // so call sites keep saying HmInstance::… for a type it still hands out — the
    // same arrangement CaptureProvenance below has always had.
    using ReferenceAnchor = pinpoint::hm::ReferenceAnchor;
    ...
    //
    // ⚠ THE TYPE LIVES WITH THE THING THAT MEASURES IT, for the same reason
    // ReferenceAnchor does: HmSessionWorker issues the request, assembles the
    // block and fills in every coverage field, and this class only moves the
    // finished result to the GUI thread. See src/IMU/hm_session_types.h for the
    // fields and what each of them is allowed to claim.
    using HistoryResult = pinpoint::hm::HistoryResult;
