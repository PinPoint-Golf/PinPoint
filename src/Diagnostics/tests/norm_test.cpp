...
int main()
{
    std::printf("=== norm: z per side ===\n");
    {
        ...
        check(near(normZ(10.0, n, Shape::Target), 0.0), "at the centre, z is zero");
        check(near(normZ(12.0, n, Shape::Target), 1.0), "one sigmaHi above -> z = +1");
        check(near(normZ(9.0,  n, Shape::Target), -1.0), "one sigmaLo below -> z = -1");
        check(near(normZ(8.0,  n, Shape::Target), -2.0), "two sigmaLo below -> z = -2");
        // The point of the asymmetry: the same absolute deviation grades differently either way.
        check(std::fabs(normZ(8.0, n, Shape::Target)) > std::fabs(normZ(12.0, n, Shape::Target)),
    ...
    std::printf("=== norm: degenerate tolerance ===\n");
    {
        ...
        check(near(normZ(5.0, n, Shape::Target), 0.0), "zero sigma: the centre itself is still z = 0");
        check(std::isinf(normZ(5.1, n, Shape::Target)), "zero sigma: anything off-centre is infinitely far");
        check(grade(5.1, n, Shape::Target) == Grade::Action, "zero sigma: off-centre grades Action, not a crash");
    }
    ...
    std::printf("=== norm: grade boundaries (no explicit monitor) ===\n");
    {
        ...
        check(grade(0.0,  n, Shape::Target) == Grade::Ideal,  "centre -> Ideal");
        check(grade(1.0,  n, Shape::Target) == Grade::Ideal,  "exactly 1 sigma -> Ideal (inclusive)");
        check(grade(-1.0, n, Shape::Target) == Grade::Ideal,  "exactly -1 sigma -> Ideal (inclusive)");
        check(grade(1.5,  n, Shape::Target) == Grade::Good,   "between 1 and 2 sigma -> Good");
        check(grade(2.0,  n, Shape::Target) == Grade::Good,   "exactly 2 sigma -> Good (inclusive)");
        check(grade(2.5,  n, Shape::Target) == Grade::Watch,  "between 2 and 3 sigma -> Watch");
        check(grade(3.0,  n, Shape::Target) == Grade::Watch,  "exactly 3 sigma -> Watch (inclusive)");
        check(grade(3.01, n, Shape::Target) == Grade::Action, "beyond 3 sigma -> Action");
        check(grade(-3.01, n, Shape::Target) == Grade::Action, "beyond -3 sigma -> Action");
    }
    ...
    std::printf("=== norm: grade policy is honoured ===\n");
    {
        ...
        check(grade(0.75, n, Shape::Target, strict) == Grade::Good,   "a tighter policy narrows Ideal");
        check(grade(1.25, n, Shape::Target, strict) == Grade::Watch,  "a tighter policy narrows Good");
        check(grade(2.0,  n, Shape::Target, strict) == Grade::Action, "a tighter policy reaches Action sooner");
        // The same value under the default policy is two bands better — the policy is genuinely
        // pack-wide and not baked into the norm.
        check(grade(2.0, n, Shape::Target) == Grade::Good, "the same value under the default policy is Good");
    }
    ...
    std::printf("=== norm: explicit monitor band dominates ===\n");
    {
        ...
        check(n.hasExplicitMonitor(Shape::Target), "both monitor bounds set -> explicit");
        check(grade(0.0, n, Shape::Target) == Grade::Ideal,   "centre -> Ideal");
        check(grade(3.0, n, Shape::Target) == Grade::Ideal,   "green edge (1 sigma) -> Ideal");
        check(grade(4.0, n, Shape::Target) == Grade::Good,    "inside the monitor band, 1.33 sigma -> Good");
        check(grade(7.0, n, Shape::Target) == Grade::Watch,   "inside the monitor band, 2.33 sigma -> Watch");
        check(grade(8.0, n, Shape::Target) == Grade::Watch,   "exactly at the monitor edge -> Watch, not Action");
        check(grade(8.5, n, Shape::Target) == Grade::Action,  "past the monitor edge -> Action even at 2.83 sigma");
        check(grade(-8.5, n, Shape::Target) == Grade::Action, "past the lower monitor edge -> Action");
    }
    ...
    std::printf("=== norm: monitor band caps Action from inside ===\n");
    {
        ...
        check(grade(5.0, n, Shape::Target) == Grade::Watch,
              "5 sigma but inside a wide monitor band -> capped at Watch");
        check(grade(10.0, n, Shape::Target) == Grade::Watch, "at the wide monitor edge -> Watch");
        check(grade(10.5, n, Shape::Target) == Grade::Action, "past the wide monitor edge -> Action");
    }
    ...
    std::printf("=== norm: asymmetric monitor (the tempo shape) ===\n");
    {
        ...
        check(grade(2.6, n, Shape::Target) == Grade::Ideal,  "centre -> Ideal");
        check(grade(2.2, n, Shape::Target) == Grade::Ideal,  "green lower edge -> Ideal");
        check(grade(3.0, n, Shape::Target) == Grade::Ideal,  "green upper edge -> Ideal");
        ...
        check(grade(1.8, n, Shape::Target) == Grade::Good,   "amber lower edge is exactly 2 sigma -> Good");
        check(grade(3.6, n, Shape::Target) == Grade::Watch,  "amber upper edge is 2.5 sigma -> Watch");
        check(grade(1.8, n, Shape::Target) != Grade::Action && grade(3.6, n, Shape::Target) != Grade::Action,
              "inside the monitor band, neither amber edge reaches Action");
        check(grade(1.7, n, Shape::Target) == Grade::Action, "below amber -> Action");
        check(grade(3.7, n, Shape::Target) == Grade::Action, "above amber -> Action");
    ...
    std::printf("=== norm: the claim, and the Ideal band it is NOT ===\n");
    {
        ...
        for (const GradePolicyPreset &p : gradePolicyPresets()) {
            const NormBandEdges e = bandEdgesOf(n, Shape::Target, p.policy);
        ...
        check(near(bandEdgesOf(n, Shape::Target, gradePolicyByName(QStringLiteral("standard"))).idealLo,
                   n.claimLo()),
              "…and coincides with the claim under standard, which is why this hid for so long");
        check(!near(bandEdgesOf(n, Shape::Target, gradePolicyByName(QStringLiteral("strict"))).idealLo,
                    n.claimLo()),
        ...
        for (const GradePolicyPreset &p : gradePolicyPresets()) {
            for (const Norm &n : rows) {
                ...
                for (double eps : { 0.0, -1e-9, -1e-3 }) {
                    check(grade(e.idealLo - eps, n, Shape::Target, p.policy) == Grade::Ideal,
                          "inside the drawn low Ideal edge grades Ideal");
                    check(grade(e.idealHi + eps, n, Shape::Target, p.policy) == Grade::Ideal,
                ...
                check(ragOf(grade(e.idealLo, n, Shape::Target, p.policy)) == PpRag::Green
                      && ragOf(grade(e.idealHi, n, Shape::Target, p.policy)) == PpRag::Green
                      && ragOf(grade(n.mu, n, Shape::Target, p.policy)) == PpRag::Green,
    ...
    std::printf("=== norm: a FLOOR grades one tail ===\n");
    {
        ...
        // to fall outside of.
        for (const GradePolicyPreset &p : gradePolicyPresets())
            check(grade(2.0, n, f, p.policy) == Grade::Ideal, p.name);

        // The single computed edge obeys the float-edge doctrine exactly as a target's does.
        for (const GradePolicyPreset &p : gradePolicyPresets()) {
            const NormBandEdges e = bandEdgesOf(n, f, p.policy, -1.0);
            ...
            check(grade(e.idealLo, n, f, p.policy) == Grade::Ideal, "ON the drawn edge is Ideal");
            check(grade(e.idealLo + 1e-9, n, f, p.policy) == Grade::Ideal, "…and just inside");
            check(grade(e.watchLo - 1e-9, n, f, p.policy) == Grade::Action, "…just outside Watch");
        }
        ...
        const NormBandEdges swept = bandEdgesOf(n, f, {}, 0.10);
    ...
    std::printf("=== norm: a CEILING is the mirror ===\n");
    {
        ...
        const NormBandEdges e = bandEdgesOf(n, c);
    ...
    std::printf("=== norm: a one-sided monitor is a COMPLETE monitor ===\n");
    {
        ...
        check(!n.hasExplicitMonitor(Shape::Target), "half a band is not a monitor on a Target");
    ...
    std::printf("=== norm: a reading nobody believes is not a grade ===\n");
    {
        ...
        m.monitorHi   = 15.0;
        m.plausibleHi = 40.0;
        check(grade(20.0, m, Shape::Target) == Grade::Action,      "past the monitor is Action");
        check(grade(50.0, m, Shape::Target) == Grade::NotMeasured, "…but past belief is not graded at all");

        // Bounds may appear singly: a floor caps above and says nothing below.
        Norm lo        = symmetric(10.0, 1.0);
        lo.plausibleLo = 0.0;
        check(grade(-1.0, lo, Shape::Target) == Grade::NotMeasured
                  && grade(99.0, lo, Shape::Target) == Grade::Action,
    ...
    std::printf("=== norm: a degenerate one-sided norm ===\n");
    {
        ...
        check(grade(0.9, n, Shape::Target) == Grade::Action && grade(1.1, n, Shape::Target) == Grade::Action,
